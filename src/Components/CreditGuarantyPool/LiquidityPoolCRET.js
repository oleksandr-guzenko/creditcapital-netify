...
import LiquidityHistory from '../LandingPage/LiquidityHistory'
...
const LiquidityPoolCRET = () => {
  ...
  const submitDepositLiquidityPool = (e) => {
    e.preventDefault()
    dispatch(
      liquidityDepositAction(depositPrice, 'deposit', 'USDC', 'CRET_TYPE')
    )
  }
  // Withdraw
  ...
  const submitWithdrawLiquidityPool = (e) => {
    dispatch(
      liquidityWithdrawAction(withdrawPrice, 'withdraw', 'CRET', 'CRET_TYPE')
    )
    e.preventDefault()
  }
  ...
  useEffect(() => {
    if (userAddress) {
      dispatch(getCoolDownPeriod('CRET_TYPE'))
    }
  }, [userAddress])

  const claimFunds = () => {
    dispatch(claimWithdraw('CRET_TYPE'))
  }
      ...
      <div className='liquidity__pool'>
        <Container>
          <div className='liquidity__pool__wrapper'>
            <h4 className='section__titles'>Liquidity Pool CRET</h4>
...
export default LiquidityPoolCRET
